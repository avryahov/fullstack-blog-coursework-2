...
## Правило истории изменений

Разработка ведется через частые атомарные коммиты.

Рабочий ориентир:
- история проекта должна быть детальной;
- один технический этап разбивается на несколько маленьких проверяемых коммитов;
- суммарная история проекта должна накапливаться до уровня не менее 100 коммитов;
- крупные пачки изменений в один commit не допускаются, если их можно разделить без потери связности.

Формат сообщений:
- в `dev` допустимы обычные префиксы вроде `feat`, `fix`, `chore`, `content`;
- в feature-ветках commit должен иметь формат `feature/<name>: message`;
- в feature-ветках второй префикс после двоеточия не используется.
